...
Running all checks can be time-consuming, especially when some preparation steps like code coverage (`covr`) or `R CMD check` (`rcmdcheck`) are slow. Instead of listing individual checks to include, you can exclude entire groups of checks by their preparation step name.

Each check in **goodpractice** depends on one or more preparation steps. The available prep names are: `covr`, `cyclocomp`, `description`, `expressions`, `lintr`, `namespace`, and `rcmdcheck`. Excluding a prep name removes all checks that depend on it.
