...
Note that `checks` takes individual check names (like `"lintr_assignment_linter"`), not prep group names.
Prep names and check names sometimes overlap — there is a check called `"covr"` and a prep called `"covr"` — but they are different things.
To find the check names in a particular group, filter `all_checks()`:

```{r}
# find all checks that start with "description"
grep("^description", all_checks(), value = TRUE)
```
