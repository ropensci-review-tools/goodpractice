...
Running all 250+ checks takes a while — not because the checks themselves are slow, but because data gathering is.
Code coverage needs to install and exercise your package.
`R CMD check` runs its full suite.
Linting reads every source file.

These data-gathering steps are called _preparations_ ("preps") — such as running `R CMD check`, computing code coverage, and linting source files.
Each prep runs once, and every check in that group draws from the same results.
When you want to skip an entire category of checks — say, coverage — you exclude the prep by name and everything that depends on it goes away.

```{r, eval = FALSE}
# skip coverage and R CMD check — just run style and metadata checks
options(goodpractice.exclude_preps = c("covr", "rcmdcheck"))
gp(".")
```

Three functions help you choose what to run:
...
`default_checks()` is the default.
`tidyverse_checks()` adds opt-in style checks.
`all_checks()` combines both.

## Check groups

Here are all the groups, identified by their prep name — the string you pass to `goodpractice.exclude_preps` when you want to skip one:
...
Checks with no prep listed run without any data gathering and cannot be excluded by prep name.
Use the `checks` argument to select them individually instead.
...
You can exclude preps using an R option or an environment variable.
Both accept the prep names from the table above.
