...
Here are the groups, identified by their prep name:
...
The checks themselves are fast — what takes time is data gathering.
When `covr` alone takes three minutes and all you care about right now is code style, why wait?

Instead of picking individual checks by name, you can exclude an entire prep — and every check that depends on it goes with it.
The prep names match the "Prep" column in the table above: `covr`, `cyclocomp`, `description`, `lintr`, `namespace`, `rcmdcheck`, `rd`, `spelling`, `urlchecker`, `available_packages`, and `tidyverse_lintr`.
