...
The `all_check_groups()` function has an additional `describe` parameter.
Calling with non-default `describe = TRUE` returns full descriptions of all
checks (modified here from default screen output to look nice):

```{r check_group-descs, eval = FALSE}
all_check_groups(describe = TRUE)
```


```{r results="asis", echo = FALSE}
descs <- all_check_groups(describe = TRUE)
df <- data.frame(Group = names(descs), Description = unlist(descs), row.names = NULL)
tbl <- knitr::kable(df)
if (knitr::is_html_output()) {
  tbl <- kableExtra::column_spec(tbl, 1, extra_css = "white-space: nowrap")
  tbl <- kableExtra::column_spec(tbl, 2, width = "100%")
}
tbl
```
